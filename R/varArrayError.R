...
#' 
#' @seealso \code{\link{zPatterns}}, \code{\link{varArray}}.
#' 
#' @examples
#' data(Water)
#' zPatterns(Water, label = 0)
#' varArrayError(Water)
#' varArrayError(Water, breakdown = TRUE)
#'
...
#' varArrayError(Water_multKM,groups=rep(c("A","B"),each=50))

varArrayError <- function(X, label = 0, groups = NULL, breakdown = FALSE, suppress.print = FALSE)
{
  if (is.vector(X))
...
## Call varArray
  VMZ <- varArray(X = X, label = label, groups = groups, suppress.print = TRUE)
